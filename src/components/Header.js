...
function Header(props) {
  ...
  return (
    <AppBar position="static" color="default" elevation={1} sx={{ backgroundColor: '#7D7F7C' }}>
      <Toolbar sx={{ borderBottom: 1, borderColor: 'divider', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Button size="small" variant="text" onClick={() => navigate('/createpost')} sx={{ marginRight: 2, typography: { fontFamily: 'YourCustomFont', fontSize: '14px', color: 'white' }}}>
            Create POST 
          </Button>
          {isAdmin && (
            <Button size="small" variant="text" onClick={() => navigate('/admin')} sx={{ marginRight: 2, typography: { fontFamily: 'YourCustomFont', fontSize: '14px', color: 'white' }}}>
              Admin controls
            </Button>
          )}
        </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          {isAuthenticated() ? (
            <Button variant="text" size="small" onClick={handleLogout} sx={{ color: 'white' }}>
              Sign-out
            </Button>
          ) : (
